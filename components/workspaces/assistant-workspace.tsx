...
function TasksManager() {
  ...
  return (
    <div className="space-y-6">
      <SectionTitle
        title="مهام الفريق"
        subtitle="تابع مهامك المسندة من الليدر، وأسند مهام جديدة لأعضاء الفريق."
        action={activeTab === 'manage' ? (
          <button
            onClick={() => setShowModal(true)}
            className="inline-flex items-center gap-2 rounded-xl bg-chart-4 px-3.5 py-2 text-sm font-semibold text-chart-4-foreground transition-transform hover:scale-[1.03]"
          >
      ...
      {/* Tab Switcher */}
      <div className="flex gap-2 rounded-2xl border border-border bg-secondary/20 p-1.5">
        <button
        ...
        >
          ...
          {myPendingAssigned.length > 0 && (
            <span className="flex size-5 items-center justify-center rounded-full bg-chart-4 text-[10px] font-bold text-chart-4-foreground">
              {myPendingAssigned.length}
            </span>
          )}
        </button>
        <button
        ...
        <div className="space-y-3">
          {myPendingAssigned.length === 0 ? (
            <Panel className="py-12 text-center">
              ...
              <p className="text-base font-bold text-foreground">رائع! لا توجد مهام معلقة مسندة لك 🎉</p>
              <p className="mt-1 text-xs text-muted-foreground">
                لم يتم إسناد مهام جديدة لك من الليدر بعد، أو قمت بإنجاز كافة مهامك.
              </p>
            </Panel>
          ) : (
            <AnimatePresence mode="popLayout">
              {myPendingAssigned.map((t) => (
                <motion.div
                  key={t.id}
                  layout
                        ...
                        <div className="mt-1.5 flex flex-wrap items-center gap-2">
                          ...
                          {t.submissionType === 'whatsapp' && t.submissionValue && (
                            ...
                            >
                              ...
                              تسليم واتساب ({t.submissionValue})
          ...
          {/* Completed assigned tasks accordion */}
          {myCompletedAssigned.length > 0 && (
            <div className="pt-2 border-t border-border/40">
              <button
                onClick={() => setShowCompletedMine(!showCompletedMine)}
                className="flex items-center gap-2 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors"
              >
                {showCompletedMine ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}
                <span>المهام المكتملة ({myCompletedAssigned.length})</span>
              </button>
              {showCompletedMine && (
                <div className="mt-3 space-y-2 opacity-80">
                  {myCompletedAssigned.map((t) => (
                    <Panel key={t.id} className="flex items-center justify-between gap-2 py-2.5 bg-secondary/20">
                      <div className="flex items-center gap-2">
